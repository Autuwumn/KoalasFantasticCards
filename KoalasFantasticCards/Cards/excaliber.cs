...
namespace KFC.MonoBehaviors
{
    public class excaliber_Mono : CardEffect
    {
        ...
        protected override void Start()
        {
            ...
            excaliberSword = escaber.GetOrAddComponent<ExcaliberSword_Mono>();
        ...
        {
            var swords = FindObjectsOfType<Rigidbody2D>().Where((g) => g.name == swordName).ToList();
            foreach(var sword in swords)
            {
                if(sword.gameObject.GetComponent<ExcaliberSword_Mono>() == null)
                {
                    sword.gameObject.AddComponent<ExcaliberSword_Mono>().player = PlayerManager.instance.players.Where((p) => p.playerID == playerid).ToArray()[0];
                    return;
                }
            }
        }
